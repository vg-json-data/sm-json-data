...
When using this data to support vanilla door colors, which are specified by their `nodeSubType`, the door is required to be unlocked with an `unlocksDoors` object if and only if there is no `locks` property with a `coloredDoor` `lockType` specified; otherwise the strats included in the `locks` should be used.
